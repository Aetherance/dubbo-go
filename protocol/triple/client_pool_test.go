...
func TestNewTriClientPoolWarmUp(t *testing.T) {
	...
	pool := NewTriClientPool(warmUpSize, maxSize, factory)
	defer pool.Close()
...
func TestTriClientPoolGetAndPutBasic(t *testing.T) {
	...
	pool := NewTriClientPool(1, 1, factory)
	defer pool.Close()
...
func TestTriClientPoolGetTimeoutFallback(t *testing.T) {
	...
	pool := NewTriClientPool(0, 1, factory)
	defer pool.Close()
	...
	start := time.Now()
	fallback, err := pool.Get(5 * time.Millisecond)
	elapsed := time.Since(start)
	if err != ErrTriClientPoolTimeout {
		t.Fatalf("second Get() error = %v, want ErrTriClientPoolTimeout", err)
	}
	if fallback == nil {
	...
	if elapsed < 5*time.Millisecond {
		t.Fatalf("Get() returned before timeout elapsed: got %v, want >= 5ms", elapsed)
	}
...
func TestTriClientPoolClose(t *testing.T) {
	pool := NewTriClientPool(1, 1, func() *tri.Client {
		return &tri.Client{}
	})
...
func benchmarkSingleClient(b *testing.B, baseTransport *http.Transport, testURL string) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			_ = client.CallUnary(ctx, req, resp)
			cancel()
		}
	})
}
...
func benchmarkTriClientPool(b *testing.B, baseTransport *http.Transport, testURL string) {
	b.SetParallelism(10)
	poolSize := 128
	pool := NewTriClientPool(16, poolSize, func() *tri.Client {
		transport := &http.Transport{
			TLSClientConfig:     baseTransport.TLSClientConfig,
			MaxIdleConns:        100,
			MaxIdleConnsPerHost: 100,
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			req := tri.NewRequest(map[string]string{"test": "data"})
			resp := tri.NewResponse(map[string]string{})
			_ = triClient.CallUnary(ctx, req, resp)
			pool.Put(triClient)
			cancel()
		}
	})
